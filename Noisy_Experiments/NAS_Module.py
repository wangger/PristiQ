...
class QLayer_10(tq.QuantumModule):
    ...
    def __init__(self, n_qubits, Not_Expanded=False, n_ori_qubits=None):
        super().__init__()
        # gates with trainable parameters
        self.n_qubits = n_qubits
        self.trainable_modules_head = nn.ModuleList()
        if Not_Expanded:
            self.para_qubits = n_ori_qubits
        else:
            self.para_qubits = self.n_qubits

        for i in range(self.para_qubits):
    ...
    def forward(self, q_device: tq.QuantumDevice):
        ...
        for index in range(self.para_qubits):
            self.trainable_modules_head[index](self.q_device, wires=index)

        # For the non-trainable entanglement (add on-the-fly)
        for offset in range(self.para_qubits - 1):
            loc_0 = self.para_qubits - 1 - offset
            loc_1 = loc_0 - 1
            tqf.cz(self.q_device, wires=[loc_0, loc_1], static=self.static_mode, parent_graph=self.graph)

        tqf.cz(self.q_device, wires=[0, self.para_qubits - 1], static=self.static_mode, parent_graph=self.graph)
...
class QLayer_13(tq.QuantumModule):
    ...
    def forward(self, q_device: tq.QuantumDevice):
        ...
        crz_index = 0
        self.trainable_modules_head_crz[crz_index](self.q_device, wires=[self.para_qubits - 1, 0])
        crz_index += 1

        for offset in range(self.para_qubits - 1):
            loc_0 = self.para_qubits - 2 - offset  # control
            loc_1 = loc_0 + 1  # target
            self.trainable_modules_head_crz[crz_index](self.q_device, wires=[loc_0, loc_1])
            crz_index += 1

        assert crz_index == self.para_qubits, "implementation error of first crz in v13"
        ...
        crz_index = 0
        f_half_beg = self.para_qubits // 2 + 1  # control_end
        l_half_end = f_half_beg - 1  # control_end

        # build the first half
        for c_index in range(f_half_beg, self.para_qubits):
        ...
        # build the last half
        self.trainable_modules_middle_crz[crz_index](self.q_device, wires=[0, self.para_qubits - 1])
        crz_index += 1
        ...
        assert crz_index == self.para_qubits, "implementation error of last crz in v13"
...
class QLayer_19(tq.QuantumModule):
    ...
    def forward(self, q_device: tq.QuantumDevice):
        ...
        crx_index = 0
        self.trainable_modules_crx[crx_index](self.q_device, wires=[self.para_qubits - 1, 0])
        crx_index += 1
        ...
        assert crx_index == self.para_qubits, "implementation error of crx in v19"
...
class QLayer_5(tq.QuantumModule):
    ...
    def __init__(self, n_qubits, Not_Expanded=False, n_ori_qubits=None):
        ...
        for i in range(self.para_qubits):
            for j in range(self.para_qubits - 1):
    ...
    def forward(self, q_device: tq.QuantumDevice):
        ...
        crz_index = 0
        for offset in range(self.para_qubits):  # offset of control end
            loc_0 = self.para_qubits - 1 - offset  # control

            # downside crz gate
            for target_down in range(self.para_qubits - 1, loc_0, -1):
        ...
        assert crz_index == self.para_qubits * (self.para_qubits - 1), "implementation error of crz in v5"
