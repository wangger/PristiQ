...
class ToQuantumData_General(object):
    """
    It could handle the image which is not square
    class ToQuantumData is to transform the input image into a unitary matrix to be encoded in quantum unitary matrix.
    """
    def __init__(self, img_size_row, img_size_col):
        super().__init__()
        # TODO(NOTE): Modification
        self.img_size_row = img_size_row
        self.image_size_col = img_size_col
    ...
    def __call__(self, tensor):
        ...
        input_matrix = input_matrix.transpose(0, 1)
        # print("-" * 50)
        # print("The shape of input matrix is ", input_matrix.shape)
        u, s, v = np.linalg.svd(input_matrix)
        # print("The shape of u, s, v is ", u.shape, s.shape, v.shape)
        output_matrix = torch.tensor(np.dot(u, v))
        # print("The shape of output matrix is ", output_matrix.shape)
        output_data = output_matrix[:, 0].view(1, self.img_size_row, self.image_size_col)  # TODO(NOTE): Modification
        # print("The shape of output data is ", output_data.shape)
        # print("-" * 50)
        return output_data
...
def load_data_mnist_general(interest_num, args, is_to_q=True, is_shuffle=True, disable_visualize=True,
                            n_test_samples=None):
    # Get the necessary input parameters
    img_size_row = int(args.img_size)
    img_size_col = int(args.img_size_col) if args.img_size_col is not None else img_size_row
    ...
    if isppd:
        # TODO:
        train_data = SelfMNIST(root=datapath, img_size=img_size_row, train=True)
        test_data = SelfMNIST(root=datapath, img_size=img_size_row, train=False)
        ...
        if is_to_q:
            ...
            # TODO: Why does the input data has no normalization?? since the resize?
            transform = transforms.Compose(
                [transforms.Resize((img_size_row, img_size_col)), transforms.ToTensor(), ToQuantumData_General(
                    img_size_row, img_size_col)])
            transform_inference = transforms.Compose(
                [transforms.Resize((img_size_row, img_size_col)), transforms.ToTensor(), ToQuantumData_General(
                    img_size_row, img_size_col)])

        else:
            transform = transforms.Compose([transforms.Resize((img_size_row, img_size_col)), transforms.ToTensor()])
            transform_inference = transforms.Compose([transforms.Resize((img_size_row, img_size_col)),
                                                      transforms.ToTensor()])
    ...
    if n_test_samples is not None:
        ...
        remained_n_test_samples = len(test_data.data) - n_test_samples
        # print("I am here", remained_n_test_samples, n_test_samples)
        # test_data, remained_test_data, indices = random_split(dataset=test_data,
        #                                              lengths=[n_test_samples, remained_n_test_samples])
        test_data, remained_test_data = random_split(dataset=test_data,
                                                     lengths=[n_test_samples, remained_n_test_samples])
    # prepare data loaders
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=batch_size,
                                               num_workers=num_workers, shuffle=is_shuffle, drop_last=False)
    test_loader = torch.utils.data.DataLoader(test_data, batch_size=inference_batch_size,
...
def load_data_fmnist_general(interest_num, args, is_to_q=True, is_shuffle=True, disable_visualize=True,
                             n_test_samples=None):
    # Get the necessary input parameters
    img_size_row = int(args.img_size)
    img_size_col = int(args.img_size_col) if args.img_size_col is not None else img_size_row
    ...
    # prepare data loaders
    # TODO(NOTE): 100 samples, bs = 64 , we do not want to drop last
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=batch_size,
                                               num_workers=num_workers, shuffle=is_shuffle, drop_last=False)
    test_loader = torch.utils.data.DataLoader(test_data, batch_size=inference_batch_size,
                                              num_workers=num_workers, shuffle=False, drop_last=False)
