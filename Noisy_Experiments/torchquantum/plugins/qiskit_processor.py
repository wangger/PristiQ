...
from torchquantum.utils import (get_expectations_from_counts, get_provider, get_prob_one_from_counts,
                                get_provider_hub_group_project, get_prob_from_counts, num2bin,
                                get_circ_stats)
...
class QiskitProcessor(object):
    ...
    def qiskit_init(self):
        ...
        if self.use_real_qc:
            ...
            # use simulator
            # print("I am in simulator")
            # print(self.noise_model_name)
            # print(self.coupling_map_name)
            # print(self.basis_gates_name)
    ...
    def amp_process_ready_circs(self, q_device, circs_all, n_output, parallel=True):
        circs_all_transpiled = []
        for circ in tqdm(circs_all):
            circs_all_transpiled.append(self.transpile(circ))
        ...
        # TODO(NOTE): It is not flipped
        n_qubits = q_device.n_wires
        if n_output <= n_qubits:
            probs_one = get_prob_one_from_counts(counts, n_qubits=n_qubits)
            measured_qiskit = probs_one[::, 0:n_output]
        elif n_output <= 2**n_qubits:
            probs_list = get_prob_from_counts(counts)  # a list of dict, like counts
            measured_qiskit = []
            for probs in probs_list:    # probs is a dict
                single_measure = []
                for amp_idx in range(n_output):
                    amp_idx_str = num2bin(amp_idx, n_qubits)
                    if amp_idx_str not in probs:
                        amp_idx_prob = 0.0
                    else:
                        amp_idx_prob = probs[amp_idx_str]
                    single_measure.append(amp_idx_prob)
                
                measured_qiskit.append(single_measure)

            measured_qiskit = np.array(measured_qiskit)
