...
def get_prob_one_from_counts(counts, n_qubits):
    ...
    res = np.stack(prob_ones)   # (bs, n_qubits)
    return res


# prob for each basis state, counts -> probs, [{}], (bs,)
def get_prob_from_counts(counts):  # counts is a list of dict
    probs = []
    if isinstance(counts, dict):
        counts = [counts]   # counts is a list of dict, [bs]
    ...
    return probs  # still a list of dict, [bs]
